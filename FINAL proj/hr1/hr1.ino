...
void loop() {
  ...
  Signal = analogRead(PulseSensorPurplePin);  // Read the PulseSensor's value.
   ...
   if(Signal > Threshold){    
     Serial.println(1);                      // If the signal is above "550", then "turn-on" Arduino's on-Board LED.
     digitalWrite(LED,HIGH);
   } else {
         Serial.println(0);                      // If the signal is above "550", then "turn-on" Arduino's on-Board LED.
